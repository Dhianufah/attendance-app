...
class StorageServices {
  ...
  final DatabaseReference _database = FirebaseDatabase.instanceFor(
    ...
    databaseURL: 'https://attendance-app-fa233-default-rtdb.asia-southeast1.firebasedatabase.app/'
  ). ref();

  // upload photo to firebase realtime database as Base64/(string)
  Future<String> uploadAtendancePhoto(String localPath, String photoType) async {
    try {
      final user = _auth.currentUser;
      // ini kalo usernya ga kedaftar
      if (user == null) throw Exception('User not authenticated'); 

      final file = File(localPath);

      //compress image to reduce size (important for realtime database)
      final compresssBytes = await FlutterImageCompress.compressWithFile(
        file.absolute.path,
        minWidth: 800,
        minHeight: 600,
        quality: 70,
      ...
      // ini buat upload
      if (compresssBytes == null) {
        throw Exception('Failed to compress image');
      }

      // convert to Base64
      final base64Image = base64Encode(compresssBytes);
