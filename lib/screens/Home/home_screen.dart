...
import 'package:attendance_app/screens/Home/attendance_card.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  AttenndanceRecord? _todayRecord;
  bool _isLoading = false;
  // ini loading ketika saat2 yang di butuhkan bukann kek baru buka langsung loading

  @override
  void initState() {
     // ini untuk mendengarkan semua kegiatan di hari itu kek misal user udah checkin/check out 
    super.initState();
  ...
  Future<void> _checkIn({String? photoPath}) async {
    ...
    try {
      String? photoKey;
      if (photoPath != null) {
        photoKey = await _storageServices.uploadAtendancePhoto(photoPath, 'checkin');
      }

      final now = DateTime.now();
      final record = AttenndanceRecord(
        id: '',
        userId: user.uid,
        checkInTime: now,
  ...
  Future<void> _checkOut({String? photoPath}) async {
    ...
    try {
      String? photoKey;
      if (photoPath != null) {
        photoKey = await _storageServices.uploadAtendancePhoto(
          photoPath, 'checkout'
        );

        final updateRecord = AttenndanceRecord(
          id: _todayRecord!.id,
